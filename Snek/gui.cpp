...
{
	ImGui_ImplWin32_WndProcHandler(window, message, wideParameter, longParameter);
	...
	case WM_SYSCOMMAND:
	{
		if ((wideParameter & 0xfff0) == SC_KEYMENU) // Disable ALT apllication menu
			return 0;
	}
	break;
...
// My additions for cleaner main file
void gui::Init(const char* windowName, const char* className) noexcept
{
	gui::CreateHWindow(windowName, className);
	gui::CreateDevice();
	gui::CreateImGui();
}
